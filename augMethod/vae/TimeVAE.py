...
from augMethod.vae.BaseVAE import BaseVariationalAutoencoder, Sampling


class TimeVAE(BaseVariationalAutoencoder):
    model_name = "TimeVAE"
    #这里的hidden_layer_sizes是什么暂时不知道
    #trend_poly也不清楚
    #custom_seas
    #use_residual_conn残差网络？
    #别的从父类中获取
    def __init__(
            self,
    ...
    def save(self, model_dir: str,data_name:str):
        os.makedirs(model_dir, exist_ok=True)
        torch.save(self.state_dict(), os.path.join(model_dir, f"{self.model_name}-{data_name}_weights.pth"))
    ...
    @classmethod
    def load(cls, model_dir: str,data_name:str) -> "TimeVAE":
        ...
        vae_model.load_state_dict(torch.load(os.path.join(model_dir, f"{cls.model_name}-{data_name}_weights.pth")))
        return vae_model


class TimeVAEEncoder(nn.Module):
    #seqlen,featdim,hiddenlayersizes,latentdim
    def __init__(self, seq_len, feat_dim, hidden_layer_sizes, latent_dim):
        ...
        #神经网络的层级结构
        #1.一维卷积根据hidden_layers_size来设置n层
        self.layers = []
        ...
        #这就是encoder的网络，然后进行重参数化，有mean和log_var；
...
class TimeVAEDecoder(nn.Module):
    #seqlen,featdim,hiddenlayersizes,latentdim,trendpoly,customseas,useresidualconn,encoderlastdensedim
...
class TrendLayer(nn.Module):
    ...
    def __init__(self, seq_len, feat_dim, latent_dim, trend_poly):
        ...
        self.feat_dim = feat_dim
        self.latent_dim = latent_dim
        #指的是他的多项式阶数（每一个特征有trendpoly阶的一个多项式）
        self.trend_poly = trend_poly
    ...
    def forward(self, z):
        self.trend_dense1.to(z.device)
        self.trend_dense2.to(z.device)
...
class SeasonalLayer(nn.Module):
    #这是一种给定时间周期的一种方法
    #通过学习不同周期，然后给每个周期中的不同季节学习一个值，然后在叠加
