...
import torch
import numpy as np
from augMethod.gan.networks import Embedder, Recovery, Generator, Discriminator, Supervisor
from augMethod.gan.utils import batch_generator, random_generator, MinMaxScaler, extract_time
...
class TimeGAN:
    def __init__(self, opt, ori_data):
        ...
        self.para = dict()
        self.para['module'] = 'gru'
        self.para['input_dim'] = self.z_dim
        self.para['hidden_dim'] = self.opt.augargs.ganargs.latent_dim
        self.para['num_layer'] = self.opt.augargs.ganargs.num_layers
        ...
        self.optim_embedder = torch.optim.Adam(self.embedder.parameters(), lr=self.opt.augargs.ganargs.learning_rate)
        self.optim_recovery = torch.optim.Adam(self.recovery.parameters(), lr=self.opt.augargs.ganargs.learning_rate)
        self.optim_generator = torch.optim.Adam(self.generator.parameters(), lr=self.opt.augargs.ganargs.learning_rate)
        self.optim_discriminator = torch.optim.Adam(self.discriminator.parameters(), lr=self.opt.augargs.ganargs.learning_rate)
        self.optim_supervisor = torch.optim.Adam(self.supervisor.parameters(), lr=self.opt.augargs.ganargs.learning_rate)
    ...
    def gen_batch(self):
        ...
        # Set training batch
        self.X, self.T = batch_generator(self.ori_data, self.ori_time, self.opt.augargs.ganargs.batch_size)
        self.X = torch.tensor(np.array(self.X), dtype=torch.float32).to(self.device)
        # Random vector generation
        self.Z = random_generator(self.opt.augargs.ganargs.batch_size, self.para['input_dim'], self.max_seq_len, self.T)
    ...
    def train_generator(self,join_train=False):
        ...
        self.G_loss = self.G_loss_U + \
                      self.opt.augargs.ganargs.gamma * self.G_loss_U_e + \
    ...
    def train_discriminator(self):
        ...
        self.D_loss = self.D_loss_real + \
                      self.D_loss_fake + \
                      self.opt.augargs.ganargs.gamma * self.D_loss_fake_e
