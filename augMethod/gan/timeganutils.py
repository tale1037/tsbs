...
from data_utils.datasets import TimeGANDataset
...
def timegan_trainer(model, data, time, args,device):
...
def timeganparamloader(args,timeganargs):
    code_dir = os.path.abspath(".")
    if not os.path.exists(code_dir):
        raise ValueError(f"Code directory not found at {code_dir}.")

    ## Data directory
    data_path = os.path.abspath("./dataset/ETTH")
    ...
    ## Output directories
    timeganargs.model_path = os.path.abspath(f"./output/{args.exp}/")
    out_dir = os.path.abspath(timeganargs.model_path)
